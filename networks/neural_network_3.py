...
class NeuralNetwork:
    ...
    def apply_gradients(self, gradient_w: list[np.ndarray], gradient_b: list[np.ndarray], eta_to_batch_size: float, lambda_to_batch_number:float):
        delta_w = [weights * (-eta_to_batch_size) for weights in gradient_w]
        delta_b = [biases * (-eta_to_batch_size) for biases in gradient_b]

        if lambda_to_batch_number == 0.0:
            regularization = [0 for _ in range(len(self.layers))]
        else:
            regularization = [layer.weights * eta_to_batch_size * lambda_to_batch_number for layer in self.layers]
            ## Using eta_to_batch_size * lambda_to_batch_number is the same as (eta * lambda) / n

        ## Apply weight gradient
        for layer_num, layer in enumerate(self.layers):
            layer.weights = layer.weights + delta_w[layer_num] - regularization[layer_num]
